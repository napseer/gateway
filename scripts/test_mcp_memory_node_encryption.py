#!/usr/bin/env python3
"""Smoke-test gateway MCP memory node encryption behavior."""
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("napseer_gateway_mcp_memory_test", script_path)
...
def run():
    ...
    assert mod.decrypt_memory_envelope("project-1", secret(), "node_content", envelope, {"full_path": "/notes/roundtrip"}) == b"hello memory"
    assert mod.decrypt_memory_envelope("project-1", secret(), "node_content", envelope, {"full_path": "/renamed/roundtrip"}) == b"hello memory"
    ...
    try:
        ...
        raise AssertionError("legacy full_path AAD envelopes must fail closed when moved")
    ...
    decrypted = mod.decrypt_node_for_return(
        {
            "id": "node-1",
            "project_id": "project-1",
    ...
    mod.active_memory_secret = lambda project_id, force_refresh=False: (_ for _ in ()).throw(RuntimeError("active memory secret is unavailable"))
    try:
    ...
    def memory_secret_response(method, path, payload=None, **kwargs):
        ...
        version = 4 if path.endswith("/versions/4") else 5
        return wrapped_memory_response(mod, "project-1", version)

    mod.request_json = memory_secret_response
    assert mod.active_memory_secret("project-1")["version"] == 5
    ...
    assert mod.decrypt_node_for_return(old_node, project_id="project-1")["content_text"] == "old version plaintext"
    ...
    assert mod.gateway_lock()["status"] == "locked"
    ...
    print("ok: gateway MCP memory node encryption smoke passed")
